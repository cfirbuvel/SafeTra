...
export function Navbar({ user }: NavbarProps) {
  ...
  return (
    <nav className="sticky top-0 z-50 w-full border-b border-white/10 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 shadow-[0_0_15px_rgba(16,185,129,0.05)]">
      <div className="container flex h-16 items-center justify-between px-4 md:px-8">
        <div className="flex items-center gap-3 cursor-pointer" onClick={() => router.push("/")}>
          <Shield className="h-7 w-7 text-primary" />
          <span className="text-2xl font-bold text-primary tracking-tight" style={{ fontFamily: "var(--font-rubik)" }}>
            SafeTra
          </span>
        </div>
